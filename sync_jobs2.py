...
def main():
    ...
    # Deactivate non-US jobs that slipped through
    try:
        result = supabase.table("jobs").update({"is_active": False}) \
            .eq("is_active", True) \
            .eq("location_state", "") \
            .is_("location_country", "null") \
            .execute()
        count = len(result.data) if result.data else 0
        if count > 0:
            print(f"  Deactivated {count} jobs with no location")
    except Exception as e:
        print(f"  Error cleaning non-US jobs: {e}")
